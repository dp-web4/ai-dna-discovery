#!/usr/bin/env python3
"""
Restart experiments with better process management
"""
...
def kill_stuck_processes():
    ...
    # Kill continuous experiment
    result = subprocess.run(['pgrep', '-f', 'continuous_experiment_engine.py'], 
                          capture_output=True, text=True)
    if result.stdout.strip():
        pid = result.stdout.strip()
        print(f"Stopping continuous experiment (PID: {pid})")
        os.kill(int(pid), signal.SIGTERM)
        time.sleep(2)
        
    # Kill any stuck timeout processes
    subprocess.run(['pkill', '-f', 'timeout.*ollama'], shell=False)
    
    print("Cleanup complete")


def start_improved_continuous_experiment():
    """Start an improved version of continuous experiment"""
    print("\nStarting improved continuous experiment...")
    
    # Create improved version with better timeout handling
...
'''
    ...
    with open('/home/dp/ai-workspace/improved_continuous_experiment.py', 'w') as f:
        f.write(improved_code)
        
    # Start the improved experiment
    subprocess.Popen(['nohup', 'python3', '/home/dp/ai-workspace/improved_continuous_experiment.py', 
                     '>', 'improved_continuous.log', '2>&1', '&'])
    
    print("Improved continuous experiment started!")


def main():
    """Clean up and restart experiments"""
    print("=== RESTARTING EXPERIMENTS ===")
    
    # 1. Kill stuck processes
    kill_stuck_processes()
    
    # 2. Give system time to clean up
    time.sleep(5)
    
    # 3. Start improved continuous experiment
    start_improved_continuous_experiment()
    
    # 4. Restart enhanced DNA experiment
    print("\nRestarting enhanced DNA experiment...")
    subprocess.Popen(['nohup', 'python3', '/home/dp/ai-workspace/enhanced_dna_experiment.py', 
                     '>', 'enhanced_dna_v2.log', '2>&1', '&'])
    
    print("\nAll experiments restarted with improved handling!")
    print("Check logs:")
    print("  - improved_continuous.log")
    print("  - enhanced_dna_v2.log")
