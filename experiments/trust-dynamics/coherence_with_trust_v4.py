#!/usr/bin/env python3
"""
Coherence Engine with Trust Dynamics V4 - CORRECTED
- Fixed edge ratio thresholds (0.3+ for normal scenes)
- Fixed motion detection sensitivity
- Fixed context state detection
- Proper trust evolution with realistic thresholds
August 12, 2025
"""
...
class CoherenceV4:
    ...
    def assess_camera_quality(self, frame):
        """
        V4: CORRECTED quality assessment based on actual measurements
        Normal scenes have edge ratio 0.3+, not 0.05
        """
        if frame is None:
            return 0.0
        ...
        # CORRECTED: Normal scenes have 0.15-0.35 edge ratio
        # Covered camera has < 0.01
        edge_score = 1.0
        if edge_ratio < 0.01:  # Very few edges - likely covered
            edge_score = edge_ratio * 100  # Scale up from 0-1%
        elif edge_ratio < 0.10:  # Few edges but not covered
            edge_score = 0.5 + (edge_ratio - 0.01) * 5.5
        elif edge_ratio > 0.40:  # Too many edges (noise)
            edge_score = max(0.3, 1.0 - (edge_ratio - 0.40) * 2)
        # 0.10-0.40 is normal range, score stays at 1.0
    ...
    def compute_camera_motion(self, frame_l, frame_r):
        """
        V4: CORRECTED motion detection with proper thresholds
        """
        motion_l = 0.0
        motion_r = 0.0
        ...
        if frame_l is not None and self.prev_frame_l is not None and self.camera_left_quality > 0.3:
            ...
            # Frame difference with higher threshold
            diff = cv2.absdiff(gray_curr, gray_prev)
            motion_pixels = np.sum(diff > 50)  # Increased threshold (was 30)
            total_pixels = diff.shape[0] * diff.shape[1]
            # Adjusted normalization - need more pixels to register as motion
            motion_l = min(motion_pixels / (total_pixels * 0.02), 1.0)  # Was 0.1
        ...
        if frame_r is not None and self.prev_frame_r is not None and self.camera_right_quality > 0.3:
            ...
            diff = cv2.absdiff(gray_curr, gray_prev)
            motion_pixels = np.sum(diff > 50)  # Increased threshold
            total_pixels = diff.shape[0] * diff.shape[1]
            motion_r = min(motion_pixels / (total_pixels * 0.02), 1.0)
    ...
    def update_context(self):
        """V4: CORRECTED context state detection with proper thresholds"""
        ...
        # CORRECTED thresholds based on actual sensor ranges
        if avg_motion < 0.02 and avg_stability > 0.98 and self.imu_motion_magnitude < 0.02:
            # Very low motion, very high stability = STABLE
            self.context_state = ContextState.STABLE
        elif avg_motion > 0.05 or self.imu_motion_magnitude > 0.1:
    ...
    def update_trust(self):
        """V4: Trust evolution with proper decay/recovery"""
        quality_threshold = 0.3  # Lowered threshold for occlusion
        trust_decay = 0.95  # Faster decay when quality is bad
        trust_recovery = 1.02  # Moderate recovery
    ...
    def compute_reality_field(self):
        ...
        # Camera contribution (motion and quality)
        camera_l_contrib = ((1.0 - self.camera_motion) * self.camera_left_quality *
                           self.trust_weights["camera_left"] * 
                           self.relevance_weights["camera_left"])
        camera_r_contrib = ((1.0 - self.camera_motion) * self.camera_right_quality *
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.use_real_imu and self.imu_data:
                ...
                self.imu_stability = 1.0 / (1.0 + gyro_mag * 0.005)  # More sensitive to motion
            else:
                # Simulated IMU
                t = time.time()
                self.imu_data["gyroscope"] = [
                ...
                self.imu_stability = 1.0 / (1.0 + gyro_mag * 0.005)
            ...
            if frame_l is not None or frame_r is not None:
                ...
                cv2.imshow("Trust Dynamics V4 - CORRECTED", dashboard)
            ...
            if key == ord('q'):
                ...
                filename = f"trust_v4_{time.strftime('%Y%m%d_%H%M%S')}.png"
    ...
    def initialize(self):
        """Initialize cameras and IMU"""
        print("\n" + "="*60)
        print("COHERENCE V4 - CORRECTED TRUST DYNAMICS")
        print("="*60)
        print("\nInitializing...")
    ...
    def create_dashboard(self, frame_l, frame_r):
        ...
        if frame_l is not None:
            ...
            color_val = int(255 * self.camera_left_quality)
            color = (0, color_val, 255 - color_val)
            cv2.rectangle(dashboard, (350, 200-quality_h), (370, 200), color, -1)
        ...
        if frame_r is not None:
            ...
            color_val = int(255 * self.camera_right_quality)
            color = (0, color_val, 255 - color_val)
            cv2.rectangle(dashboard, (730, 200-quality_h), (750, 200), color, -1)
