# AI-DNA 2.0 — Semantic Closure Horizons and Cross-Architecture Invariants
...
## The semantic unit: a semantic-closure-bounded concept state
...
AI-DNA 2.0 should instead define its unit as a **semantic closure horizon (SCH)**:

> **A semantic representation is the latent state associated with the smallest evidence/context horizon sufficient to instantiate a coherent concept, relation, or meaning for the current task.**
...
## Terminology boundary with Web4 MRH

This work was initially described using MRH language because both ideas share a relevance-scoping intuition. HUB's code/spec review established that the operational Web4 object called **MRH** is different:

- Web4 MRH is a declared, signed set of graph relationships (`bound`, `paired`, `witnessing`) plus a graph traversal depth.
- Its `horizon_depth` controls how far the graph is traversed; it does not mean "how much evidence is sufficient."
- The AI-DNA object is inferred from model behavior under progressively sufficient evidence/context.

To avoid silent vocabulary drift, AI-DNA 2.0 calls its object **semantic closure horizon (SCH)**.

The conceptual ancestry remains useful: both constrain what is relevant. The mechanisms and operational meanings remain explicitly separate.
...
- irrelevant surrounding context outside the semantic closure horizon.
...
**Question:** Does the latent representation change when meaning-relevant evidence enters the semantic closure horizon?
...
> irrelevant evidence outside the semantic closure horizon should have little effect; meaning-changing evidence inside the horizon should have a large, structured effect.
...
## Semantic-closure probe families
...
| Outside-closure noise | irrelevant context changes | state stable |
| Inside-closure evidence | meaning-relevant context added | state changes appropriately |
...
The semantic closure horizon itself may be an architecture-independent observable.
...
Semantic closure provides a way to test that directly.
...
> **Phase C1: semantic-closure invariance/sensitivity experiment.**
...
- what context lies inside versus outside its semantic closure horizon;
